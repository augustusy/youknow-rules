...
def genClash():
    ...
    for row in points:
        ...
        if len(row) > 0:
            ...
            if "type: vmess" in row and "port: 443" in row:   
                ...
                row = row + ', method=aes-128-gcm, tls13=true, obfs=wss, fast-open=false, udp-relay=false, tag=VM-' + str(j) + '(wss)'
                pointsvm.append(row)
    ...
    ## 复制到同步目录
    shutil.copy ('Output/ss-clash.txt', '../../sub/ss-clash.txt')
    shutil.copy ('Output/vmess-clash.txt', '../../sub/vmess-clash.txt')
