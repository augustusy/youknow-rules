...
def getandgeneasylistCN():
    ...
    for line in tmpf1.readlines():
        if ("$" in line) or ("#" in line) or ("*" in line): #如果包含特殊字符就跳过
            continue
        line = line.replace('^','')	
        tmpf2.write(line)
...
def getandgeneasylist():
    ...
    baseurl = 'https://raw.fastgit.org/easylist/easylist/master/easylist/easylist_adservers.txt'
    ...
    tmpf2 = codecs.open(tmpfile2, 'w', 'utf-8')
    for line in tmpf1.readlines():
        if ("$" in line) or ("#" in line) or ("*" in line): #如果包含c就跳过
            continue
        line = line.replace('^','')	
        tmpf2.write(line)
...
def getandgenuBlockfilters():
    ...
    baseurl = 'https://raw.fastgit.org/uBlockOrigin/uAssets/master/filters/filters.txt'
